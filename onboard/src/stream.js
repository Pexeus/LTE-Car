...
function initiate(server, config) {
    ...
    var stream
    var command = `raspivid -w ${config.video.width} -h ${config.video.height} -t 0 -fps ${fps} -ih -b ${bitrate} -pf baseline -mm average -ISO 800 -awb off -awbg 1.0,2.5 -ex fixedfps -ev 0 -co 50 -br ${config.video.brightness} -o - | socat - udp-sendto:${config.host}:${config.port_udp},shut-none`
